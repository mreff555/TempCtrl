 ...
 * data.                                      *
 **********************************************/
DataManager::DataManager(std::shared_ptr<ButtonManager> _buttonManager,
                         std::shared_ptr<LcdScreen> _lcdScreen)
    : mButtonManager(_buttonManager), mLcdScreen(_lcdScreen)
{
...
TScale_E DataManager::getTempScale() const { return mTempScale; }

void DataManager::setTempScale(const TScale_E tScale) { mTempScale = tScale; }

void DataManager::nextTempScale()
{
  int temp;
  temp = (int)mTempScale + 1;
  if (temp >= (int)MAX_VALUE_TSCALE)
  {
    temp = 0;
  }
...
InputMode_E DataManager::getInputMode() const { return mInputMode; }
...
void DataManager::nextInputMode()
{
  ...
  if (temp >= (int)INPUT_MODE_MAX_VALUE)
  {
    temp = 0;
  }
...
float DataManager::getSetPoint() const { return setPoint; }
